...
set old_executable = /lcrc/group/acme/ac.zguo/E3SM_simulations/anvil.0703.tuner0706_rp.ne30pg2_r05_oECv3/build/e3sm.exe
...
$xmlchange_exe --id CAM_CONFIG_OPTS --append --val="-nadv 40 -clubb_sgs -microphys mg$MGVER -psubcols $NUMSC -cppdefs '-DUWM_MISC -DSILHS'"
...
clubb_pdf_component_stdev_factor_w = 6.5
clubb_mu               = 0.0005
clubb_nu1              = 10
clubb_nu2              = 1
clubb_nu8              = 60
...
prc_exp                 = 3.0   !! same as in default
...
set i = 1
while ( $i <= $npara )
  sed -n $j,${j}p /home/ac.zguo/E3SM_code/scripts/data/senstest.dat > parameter.doc
  set values=`cat parameter.doc`
  set var_val=`echo "scale=9;${values[$i]}"|bc`
  set ${parameter[$i]} = $var_val
  echo ${parameter[$i]} = $var_val  >>user_nl_eam
@ i++
end
rm -f *.doc
### Assume we already have a table senstest.dat which contains the all parameter values for each samples.
### NOTE, the format of senstest.dat should be number of samples x the number of paras. 
...
#    cp -fp $old_executable $case_build_dir/
     ln -s $old_executable $case_build_dir/ 
  else
